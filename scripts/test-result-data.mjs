...
const experiment = runExperiment(data, {
  location: "PLAT:BET:S14:EB",
  from: 22,
  to: 23,
  capacity: 0,
});
for (const scenario of ["A", "B", "C"]) {
  for (const result of [solve(data, scenario), experiment.results[scenario]]) {
    ...
    assert.equal(
      +statuses.reduce((sum, s) => sum + s.delayPenalty, 0).toFixed(3),
      result.report.priority_weighted_score,
    );
    assert.equal(statuses.filter((s) => s.complete).length, result.report.complete);
    assert.equal(
      capacityRows(data, result).reduce((sum, r) => sum + r.excess, 0),
...
console.log(
  "PASS: per-activity delay, workload, capacity totals and experiment overrides reconcile with reports in all three policies.",
);
