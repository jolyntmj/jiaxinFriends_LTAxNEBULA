...
for (const s of ["A", "B", "C"]) {
  ...
  assert.equal(earlier.results[s].report.objective_score, s === "B" ? 10 : 7);
  assert(earlier.results[s].inputRevision.description.includes("W22 → W20"));
  assert(
    compareActivities(baseline[s], earlier.results[s]).some(
      (row) => row.id === "A036" && (row.shift < 0 || row.ecloAfter < row.ecloBefore),
    ),
  );
}
...
assert.equal(
  model.capacity(location, 21),
  Number(data["04_LOCATION_SUPPLY"].find((row) => row.location_id === location).supply_capacity),
);
assert.equal(model.capacity(location, 24), model.capacity(location, 21));
assert(validate(model, baseline.A).hard_violations.some((v) => v.rule === "capacity"));
const disruption = runExperiment(data, change);
assert.equal(disruption.results.A.report.feasible, true);
assert(
  !disruption.results.A.occupancy.some(
    (row) => row.location_id === location && row.week >= 22 && row.week <= 23,
  ),
);
for (const s of ["A", "B", "C"]) {
  assert.deepEqual(validate(model, disruption.results[s]), disruption.results[s].report);
  assert.deepEqual(
    disruption.results[s].inputRevision.capacityOverrides,
    modified.options.capacityOverrides,
  );
}
...
console.log(
  "PASS: A/B/C start-date experiments, bounded capacity disruptions, independent revalidation, affected activities, immutable baselines and invalid-input rejection.",
);
