...
/**
 * Rebuild derived fields and revalidate a proposed access/occupancy edit.
 * A recommendation is never accepted solely because its score looks lower.
 */
export function evaluatePlan(model, original, access, occupancy) {
  ...
  const results = original.results.map((row) => {
    ...
    return {
      ...
      overrun_days: Math.max(0, Math.round((Date.parse(completion) - due) / MILLISECONDS_PER_DAY)),
    };
  });
  const candidate = {
    ...original,
    ...
    occupancy,
    results,
    last: Math.max(0, ...rows.map((row) => row.week)),
    decisions: {},
    weekPlans: [],
    windows: {},
    method: "Validated local recommendation from the original schedule",
  };
...
/**
 * Search bounded local changes that remain valid under the original instance.
 * @returns {object|null} Best validated improvement, or null if none was found.
 */
export function findRecommendation(data, baseline, { maxEvaluations = 3000 } = {}) {
  ...
  const consider = (access, occupancy, action) => {
    ...
    if (candidate.report.feasible && saving > 0.000001 && (!best || saving > best.saving)) {
      best = {
        action,
        saving,
        originalScore: baseline.report.objective_score,
        suggestedScore: candidate.report.objective_score,
        candidate,
      };
    }
  };
  ...
  const activities = [...model.acts].sort((a, b) => {
    const lateness = (item) =>
      Math.max(
        0,
        ...baseline.access
          .filter((r) => r.activity_id === item.activity_id)
          .map((r) => r.week - item.p.due),
      ) * item.weight;
  ...
  for (const activity of activities) {
        ...
        for (let i = 0; i < rows.length - 1; i++) {
          for (let j = i + 1; j < rows.length - 1; j++) {
            ...
            const access = baseline.access
              .filter((r) => r !== last)
              .map((r) => (r === rows[i] || r === rows[j] ? { ...r, eclo: 1 } : r));
            const occupancy = baseline.occupancy.filter(
              (r) => !(r.activity_id === id && r.week === last.week),
            );
            consider(
              access,
              occupancy,
              `${id}: use ECLO in W${rows[i].week} and W${rows[j].week}; remove the W${last.week} access and finish in W${rows.at(-2).week}.`,
            );
          }
        }
    }
    for (const row of [...rows].reverse()) {
      if (row.eclo)
        consider(
          baseline.access.map((r) => (r === row ? { ...r, eclo: 0 } : r)),
          baseline.occupancy,
          `${id}: use a standard access instead of ECLO in W${row.week}.`,
        );
      for (let week = activity.start; week < row.week; week++) {
        if (rows.some((r) => r.week === week)) continue;
        const groups = [
          ...new Set(
            baseline.occupancy.filter((r) => r.week === week).map((r) => r.co_share_group),
          ),
        ];
        ...
        for (const group of groups)
          for (let night = 1; night <= activity.p.cap; night++) {
            if (evaluated >= maxEvaluations) return best;
            const access = baseline.access.map((r) =>
              r === row ? { ...r, week, access_night: night } : r,
            );
            const occupancy = baseline.occupancy.map((r) =>
              r.activity_id === id && r.week === row.week
                ? { ...r, week, co_share_group: group }
                : r,
            );
            consider(
              access,
              occupancy,
              `${id}: shift the W${row.week} access forward to W${week}.`,
            );
          }
      }
    }
...
/**
 * Suggest an earlier input start date only when original-instance moves do not help.
 * The returned candidate is explicitly a revised-input what-if, not a baseline submission.
 */
export function findStartDateSuggestion(data, baseline) {
  if (!baseline.report.feasible || baseline.report.objective_score <= 0) return null;
  let best = null;
  const targets = baseline.modelInfo.activities
    .filter((activity) => {
      const rows = baseline.access.filter((row) => row.activity_id === activity.id);
      return (
        rows.some((row) => row.eclo) || Math.max(...rows.map((row) => row.week)) > activity.due
      );
    })
    .slice(0, 8);
  for (const activity of targets) {
    for (let weeks = 1; weeks <= Math.min(3, activity.start - 1); weeks++) {
      ...
      row.planned_start_date = new Date(
        Date.parse(previous) - weeks * DAYS_PER_WEEK * MILLISECONDS_PER_DAY,
      )
        .toISOString()
        .slice(0, 10);
      const candidate = solve(revised, baseline.scenario);
      const saving = +(baseline.report.objective_score - candidate.report.objective_score).toFixed(
        3,
      );
      if (!candidate.report.feasible || saving <= 0 || (best && saving <= best.saving)) continue;
      const changed = baseline.modelInfo.activities
        .filter((a) => {
        ...
        .map((a) => a.id);
      const delayed = baseline.modelInfo.activities
        .filter((a) => {
          const finish = (result) =>
            Math.max(...result.access.filter((r) => r.activity_id === a.id).map((r) => r.week));
      ...
      candidate.inputRevision = {
        ...
        previous,
        proposed: row.planned_start_date,
        files: Object.fromEntries(
          Object.entries(revised).map(([name, rows]) => [name + ".csv", csv(rows)]),
        ),
      };
      best = {
        saving,
