...
export function createExperimentPanel(api) {
  ...
  panel.addEventListener("change", (event) => {
    ...
    if (event.target.id === "experiment-activity" && draft.activity) {
      draft.startWeek = state.originals.A.modelInfo.activities.find(
        (a) => a.id === draft.activity,
      ).start;
    }
    if (event.target.id === "experiment-location" && draft.location) {
      draft.capacity = Number(
        state.data["04_LOCATION_SUPPLY"].find((row) => row.location_id === draft.location)
          .supply_capacity,
      );
    }
    render();
  ...
  panel.addEventListener("click", (event) => {
    ...
    if (action === "suggestion") {
      ...
      if (!suggestion.candidate.inputRevision?.activityId) {
        experiment = {
          results: { ...state.originals, [state.scenario]: suggestion.candidate },
          description: suggestion.action,
        };
        api.show(experiment.results);
        api.message(
          `Suggestion applied to scenario ${state.scenario}. Other policies retain their baselines.`,
        );
        return;
      }
      const activity = suggestion.candidate.modelInfo.activities.find(
        (a) => a.id === suggestion.candidate.inputRevision.activityId,
      );
  ...
  function run(change) {
    ...
    try {
      buildExperiment(state.data, change);
    } catch (error) {
    ...
    try {
      ...
      finish();
      render();
      api.message(`Experiment could not start: ${error.message}`, true);
      return;
    }
    job.onmessage = ({ data: response }) => {
      if (response.type === "progress")
        api.message(`Checking scenario ${response.scenario} against the experiment…`);
      ...
      if (response.type === "done") {
        ...
        api.show(experiment.results);
        api.message(
          "Comparison ready. Your original baseline is preserved; the workspace now shows the experiment.",
        );
      }
    };
    job.onerror = (error) => {
      finish();
      render();
      api.message(`Experiment failed: ${error.message}`, true);
    };
    try {
  ...
  function render() {
    ...
    if (!ready) {
      panel.innerHTML =
        '<div class="lab-heading"><div><p class="eyebrow">PLAN COMPARISON</p><h2>See what changes before changing the plan.</h2><p class="muted">Generate a baseline to test a disruption or an earlier activity start.</p></div></div>';
      return;
    }
    const baseline = state.originals[state.scenario];
    const active =
      experiment && state.results[state.scenario] === experiment.results[state.scenario];
    ...
    panel.innerHTML = `<div class="lab-heading"><div><p class="eyebrow">PLAN COMPARISON</p><h2>Test changes. Compare outcomes.</h2><p class="muted">Use a suggested change or define your own here. Your original plan stays available.</p></div><span class="badge ${active ? "experiment-badge" : ""}">${active ? "Viewing experiment" : "Viewing baseline"}</span></div>
      ...
      <label>Activity start<select id="experiment-activity"><option value="">Keep activity starts</option>${options(
        activities.map((a) => [a.id, `${a.id} · ${a.contract} · current W${a.start}`]),
        draft.activity,
      )}</select></label>
      ...
      <label>Location capacity<select id="experiment-location"><option value="">Keep location capacity</option>${options(
        state.data["04_LOCATION_SUPPLY"].map((row) => [row.location_id, row.location_id]),
        draft.location,
      )}</select></label>
      ...
      <div id="experiment-comparison" aria-live="polite">${experiment ? comparison(state, active) : ""}</div>`;
  }
  ...
  function comparison(state, active) {
    ...
    const breakdown = (report, scenario) =>
      `${scenario === "B" ? "0 (not scored)" : fmt(report.priority_weighted_score)} delay + ${scenario === "A" ? 0 : EXTRA_ACCESS_PENALTY * report.excess_access_nights_total} extra access + ${scenario === "A" ? 0 : ECLO_PENALTY * report.eclo_nights_total} extended hours`;
    return `<div class="comparison-heading"><div><p class="eyebrow">COMPARISON READY</p><h3>${esc(experiment.description)}</h3></div></div>
      ${table(
        ["Policy", "Original penalty", "Experiment penalty", "Change", "Experiment checks"],
        ["A", "B", "C"].map((s) => {
          const before = state.originals[s].report;
          const after = experiment.results[s].report;
          return [
            `<b>${s} · ${policies[s]}</b>`,
            fmt(before.objective_score),
            after.feasible ? fmt(after.objective_score) : "Not feasible",
            before.feasible && after.feasible
              ? signed(after.objective_score - before.objective_score)
              : "—",
      ...
      <details><summary>See affected activities · ${changes.length} changed</summary>${table(
        [
          "Activity",
          "Contract",
          "Original finish",
        ...
        changes.map((row) => [
          esc(row.id),
          esc(row.contract),
          row.before ? "W" + row.before : "Unscheduled",
          row.after ? "W" + row.after : "Unscheduled",
          row.shift === null
            ? "Incomplete"
            : row.shift === 0
              ? "Access pattern changed"
              : `${Math.abs(row.shift)} weeks ${row.shift < 0 ? "earlier" : "later"}`,
          `${row.ecloBefore} → ${row.ecloAfter}`,
        ]),
      )}</details>
