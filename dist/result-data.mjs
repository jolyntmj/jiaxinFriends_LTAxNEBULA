import { accessUnits, DAYS_PER_WEEK, MILLISECONDS_PER_DAY } from "./rules.mjs";
export const weekDate = (result, week, end = false) =>
  new Date(
    Date.parse(result.modelInfo.start) +
      ((week - 1) * DAYS_PER_WEEK + (end ? DAYS_PER_WEEK - 1 : 0)) * MILLISECONDS_PER_DAY,
  )
    .toISOString()
    .slice(0, 10);
...
export function activityStatus(result, activity, data) {
  ...
  const units = rows.reduce((sum, row) => sum + accessUnits(row), 0);
  ...
  const project = data["07_PROJECT_DETAILS"].find(
    (row) =>
      row.contract_number === activity.contract && row.activity_type === input?.activity_type,
  );
  const dueDate = project?.planned_completion_date || weekDate(result, activity.due, true);
  const lateDays = finish
    ? Math.max(
        0,
        Math.round(
          (Date.parse(weekDate(result, finish, true)) - Date.parse(dueDate)) / MILLISECONDS_PER_DAY,
        ),
      )
    : 0;
  return {
    ...
    dueDate,
    lateDays,
    complete: units >= activity.workload,
    delayPenalty: lateDays * activity.weight,
    extended: rows.filter((row) => row.eclo).length,
  };
}
...
export function capacityAt(data, result, location, week) {
  const override = result.inputRevision?.capacityOverrides?.find(
    (change) => change.location === location && week >= change.from && week <= change.to,
  );
  return (
    override?.capacity ??
    Number(
      data["04_LOCATION_SUPPLY"].find((row) => row.location_id === location)?.supply_capacity || 0,
    )
  );
}
export function capacityRows(data, result) {
  ...
  return [...groups.values()]
    .map((row) => {
      ...
      return {
        ...row,
        used: row.groups.size,
        capacity,
        excess: Math.max(0, row.groups.size - capacity),
      };
    })
    .sort(
      (a, b) =>
